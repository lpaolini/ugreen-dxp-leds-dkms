...
static int ugreen_led_get_state(
    ...
    if (rc != sizeof(buf)) {
        pr_err("%s: short i2c_smbus_read_i2c_block_data for id %d: %d bytes",
                __func__, led_id, rc);
        return -EIO;
    }
...
static int ugreen_led_probe(struct i2c_client *client) {
    ...
    for (int i = 0; i < UGREEN_MAX_LED_NUMBER; ++i) {
        ...
        if (rc) {
            pr_err("failed to register led id %d: %d\n", i, rc);
            while (--i >= 0) {
                state = priv->state + i;
                if (state->registered) {
...
static int __init ugreen_led_init(void) {
    ...
    return i2c_add_driver(&ugreen_led_driver);
}
